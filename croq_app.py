...
def gerar_resumo(transcricao):
    """Função para gerar um resumo da transcrição usando a API do Groq via OpenAI"""
    if transcricao:
        # Salvar a chave atual da OpenAI
        openai_key_original = openai.api_key
        openai_base_original = openai.api_base
        
        try:
            # Configurar temporariamente para usar Groq
            openai.api_key = groq_api_key
            openai.api_base = "https://api.groq.com/openai/v1"
            
            # Fazer a chamada para gerar o resumo
            ...
            # Restaurar configuração original da OpenAI
            openai.api_key = openai_key_original
            if hasattr(openai, 'api_base'):
                openai.api_base = openai_base_original
    return None

def main():
    """Função principal da aplicação"""
    st.title("✨ Transcript AI com Groq")
    ...
    # Rodapé
    st.markdown("---")
    st.markdown("Desenvolvido por **Fernando Horas** com 💙 usando Streamlit, OpenAI e Groq")
